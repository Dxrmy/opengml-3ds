    ...
    if (is_resource(id) && g_init)
    {
        ...
        if (iter == g_resources.end())
        {
            ...
            if (sound)
            {
                ...
                resource.m_sample = std::unique_ptr<SoLoud::AudioSource>(audio_source);
#elif defined(__3DS__)
                // Read file directly into linear memory for NDSP
                FILE* f = fopen(sound->m_path.c_str(), "rb");
                if (f) {
                    fseek(f, 0, SEEK_END);
                    resource.m_linear_pcm_size = ftell(f);
                    fseek(f, 0, SEEK_SET);

                    // Simple WAV parsing - skip header for now, assume 44.1khz 16bit PCM
                    if (resource.m_linear_pcm_size > 44) {
                        fseek(f, 44, SEEK_SET);
                        resource.m_linear_pcm_size -= 44;
                        resource.m_linear_pcm_data = linearAlloc(resource.m_linear_pcm_size);
                        if (resource.m_linear_pcm_data) {
                            fread(resource.m_linear_pcm_data, 1, resource.m_linear_pcm_size, f);
                            DSP_FlushDataCache(resource.m_linear_pcm_data, resource.m_linear_pcm_size);
                        }
                    }
                    fclose(f);
                }
...
bool instance_is_playing(sound_id_t id)
{
    ...
    if (instance_t* instance = get_instance(id))
    {
        ...
        #elif defined(__3DS__)
        if (instance->m_ndsp_channel >= 0) {
            return ndspChnIsPlaying(instance->m_ndsp_channel);
        }
        #endif
    }
    return false;
...
bool instance_is_paused(sound_id_t id)
{
    ...
    if (instance_t* instance = get_instance(id))
    {
        ...
        #elif defined(__3DS__)
        if (instance->m_ndsp_channel >= 0) {
            return ndspChnIsPaused(instance->m_ndsp_channel);
        }
        #endif
    }
    return false;
...
bool instance_stop(sound_id_t id)
{
    ...
    if (instance_t* instance = get_instance(id))
    {
        ...
        #elif defined(__3DS__)
        if (instance->m_ndsp_channel >= 0) {
            ndspChnReset(instance->m_ndsp_channel);
            g_ndsp_channels_used &= ~(1 << instance->m_ndsp_channel);
            instance->m_ndsp_channel = -1;
        }
        #endif
    }
    return false;
...
bool instance_pause(sound_id_t id)
{
    ...
    if (instance_t* instance = get_instance(id))
    {
        ...
        #elif defined(__3DS__)
        if (instance->m_ndsp_channel >= 0) {
            ndspChnSetPaused(instance->m_ndsp_channel, true);
        }
        #endif
    }
    return false;
...
bool instance_resume(sound_id_t id)
{
    ...
    if (instance_t* instance = get_instance(id))
    {
        ...
        #elif defined(__3DS__)
        if (instance->m_ndsp_channel >= 0) {
            ndspChnSetPaused(instance->m_ndsp_channel, false);
        }
        #endif
    }
    return false;
...
void ogm::interpreter::fn::audio_play_sound(VO out, V audio, V p, V l)
{
    ...
    if (resource && g_init)
    {
        ...
        if (resource->m_sample)
        {
            ...
            if (loop) soloud.setLooping(handle, loop);
        ...
        if (resource->m_linear_pcm_data) {
            // Find free channel
            int channel = -1;
            for (int i = 0; i < MAX_NDSP_CHANNELS; i++) {
                if (!(g_ndsp_channels_used & (1 << i))) {
                    channel = i;
                    g_ndsp_channels_used |= (1 << i);
                    break;
                }
            }
            ...
            if (channel >= 0) {
                ...
                instance_t& instance = g_instances[g_next_instance_id];
                instance.m_ndsp_channel = channel;
                instance.m_resource_id = id;
                resource->m_instances.emplace(g_next_instance_id);

                memset(&instance.m_wavebuf, 0, sizeof(ndspWaveBuf));
                instance.m_wavebuf.data_vaddr = resource->m_linear_pcm_data;
                instance.m_wavebuf.nsamples = resource->m_linear_pcm_size / 4; // 16-bit stereo = 4 bytes per sample
                instance.m_wavebuf.looping = loop;

                DSP_FlushDataCache(resource->m_linear_pcm_data, resource->m_linear_pcm_size);
                ndspChnWaveBufAdd(channel, &instance.m_wavebuf);
